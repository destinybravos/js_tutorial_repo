...
function convert() {
    ...
    if(task === 'n2e'){
        ...
        if(nar == ''){
            ...
            eur = convertnaira2euro(nar);
            document.getElementById('eInput').value = eur;//print the dollar to the dollar input.
        }
    }else if(task === 'e2n'){
        // Dollar to Naira Conversion
        eur = document.getElementById('eInput').value; //get the value of dollar
        if(eur == ''){  //checking if dollar is empty
            alert('The euro Value is empty! Please enter euro value.');
        }else{ //if not empty
            nar = convert2Naira(eur); // call the convert to naira function and pass the value of dollar to it.
            document.getElementById('nInput').value = nar;  //print the naira to the naira input.
        }
    } 
...
document.getElementById('sInputA').addEventListener('keyup', function () {
    ...
    if(aSSvalue === aFSvalue ){
        aFi.value = theAmount;
    }else if(aSSvalue === 'dValue' && aFSvalue === 'nValue'){
        aFi.value = convert2Naira(theAmount);
    }
});
